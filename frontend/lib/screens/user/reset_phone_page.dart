import 'package:Qurbani/services/api_client.dart';
import 'package:Qurbani/services/service_profile.dart';
import 'package:Qurbani/theme/theme.dart';
import 'package:Qurbani/utils/logger.dart';
import 'package:flutter/material.dart';
import 'package:fluttertoast/fluttertoast.dart';
import 'package:intl_phone_field/intl_phone_field.dart';

class ResetPhonePage extends StatefulWidget {
  final String currentPhone;
  final String currentCountryCode;

  const ResetPhonePage({
    super.key,
    required this.currentPhone,
    required this.currentCountryCode,
  });

  @override
  State<ResetPhonePage> createState() => _ResetPhonePageState();
}

class _ResetPhonePageState extends State<ResetPhonePage> {
  final _formKey = GlobalKey<FormState>();
  final _currentPhoneController = TextEditingController();
  final _newPhoneController = TextEditingController();
  final _confirmPhoneController = TextEditingController();
  final _currentPasswordController = TextEditingController();

  String? _newCountryCode;
  String? _newPhoneNumber;
  String? _confirmCountryCode;
  String? _confirmPhoneNumber;
  bool _isLoading = false;
  bool _obscureCurrentPassword = true;

  @override
  void initState() {
    super.initState();
    _currentPhoneController.text = _currentDisplayPhone;
  }

  @override
  void dispose() {
    _currentPhoneController.dispose();
    _newPhoneController.dispose();
    _confirmPhoneController.dispose();
    _currentPasswordController.dispose();
    super.dispose();
  }

  String get _currentDisplayPhone {
    final countryCode = widget.currentCountryCode.trim();
    final phone = widget.currentPhone.trim();
    if (countryCode.isEmpty) return phone;
    if (phone.isEmpty) return countryCode;
    return "$countryCode $phone";
  }

  String _normalized(String? countryCode, String? phone) {
    return "${countryCode ?? ''}${phone ?? ''}".replaceAll(RegExp(r'\s+'), '');
  }

  Future<void> _updatePhone() async {
    if (!_formKey.currentState!.validate()) return;

    final newValue = _normalized(_newCountryCode, _newPhoneNumber);
    final confirmValue = _normalized(_confirmCountryCode, _confirmPhoneNumber);
    final currentValue = _normalized(
      widget.currentCountryCode,
      widget.currentPhone,
    );

    if (newValue != confirmValue) {
      Fluttertoast.showToast(
        msg: "Phone numbers do not match",
        toastLength: Toast.LENGTH_LONG,
        gravity: ToastGravity.BOTTOM,
        backgroundColor: AppTheme.warningRed,
        textColor: AppTheme.bgGradientEnd,
      );
      return;
    }

    if (newValue == currentValue) {
      Fluttertoast.showToast(
        msg: "New phone number must be different from current phone number",
        toastLength: Toast.LENGTH_LONG,
        gravity: ToastGravity.BOTTOM,
        backgroundColor: AppTheme.warningRed,
        textColor: AppTheme.bgGradientEnd,
      );
      return;
    }

    setState(() => _isLoading = true);

    try {
      await ProfileService.updatePhone(
        currentPassword: _currentPasswordController.text.trim(),
        countryCode: _newCountryCode!,
        newPhone: _newPhoneNumber!,
      );

      Fluttertoast.showToast(
        msg: "Phone number updated successfully!",
        toastLength: Toast.LENGTH_LONG,
        gravity: ToastGravity.BOTTOM,
        backgroundColor: AppTheme.primaryGreen,
        textColor: AppTheme.bgGradientEnd,
      );

      if (mounted) Navigator.pop(context, true);
    } catch (e, stack) {
      AppLogger.error("Phone update failed", e, stack);
      final message = e is ApiException
          ? e.message
          : e.toString().replaceFirst(RegExp(r'^(Exception|Error):\s*'), '');

      Fluttertoast.showToast(
        msg: message.isEmpty ? "Failed to update phone number" : message,
        toastLength: Toast.LENGTH_LONG,
        gravity: ToastGravity.BOTTOM,
        backgroundColor: AppTheme.warningRed,
        textColor: AppTheme.bgGradientEnd,
      );
    } finally {
      if (mounted) setState(() => _isLoading = false);
    }
  }

  InputDecoration _decoration({
    required String label,
    required IconData icon,
    Widget? suffixIcon,
  }) {
    return InputDecoration(
      filled: true,
      fillColor: AppTheme.bgGradientEnd,
      prefixIcon: Icon(icon, color: AppTheme.primaryGreen),
      labelText: label,
      labelStyle: const TextStyle(color: AppTheme.primaryGreen),
      border: const OutlineInputBorder(
        borderRadius: BorderRadius.all(Radius.circular(12)),
        borderSide: BorderSide(color: AppTheme.primaryGreen),
      ),
      focusedBorder: const OutlineInputBorder(
        borderRadius: BorderRadius.all(Radius.circular(12)),
        borderSide: BorderSide(color: AppTheme.primaryGreen),
      ),
      suffixIcon: suffixIcon,
    );
  }

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        backgroundColor: AppTheme.primaryGreen,
        title: const Text(
          "Change Phone Number",
          style: TextStyle(color: AppTheme.bgGradientEnd),
        ),
        leading: IconButton(
          icon: const Icon(Icons.arrow_back, color: AppTheme.bgGradientEnd),
          onPressed: () => Navigator.pop(context),
        ),
      ),
      body: Padding(
        padding: const EdgeInsets.all(20.0),
        child: Form(
          key: _formKey,
          child: ListView(
            children: [
              const Text(
                "Enter your current password and new phone number to update your account.",
                style: TextStyle(fontSize: 16, color: AppTheme.primaryGreen),
              ),
              const SizedBox(height: 30),
              TextFormField(
                controller: _currentPhoneController,
                readOnly: true,
                decoration: _decoration(
                  label: "Current Phone Number",
                  icon: Icons.phone_outlined,
                ),
              ),
              const SizedBox(height: 20),
              IntlPhoneField(
                controller: _newPhoneController,
                initialCountryCode: 'IN',
                keyboardType: TextInputType.phone,
                disableLengthCheck: false,
                decoration: _decoration(
                  label: "New Phone Number",
                  icon: Icons.phone_android,
                ),
                onChanged: (phone) {
                  _newCountryCode = phone.countryCode;
                  _newPhoneNumber = phone.number;
                },
                validator: (phone) {
                  if (phone == null || phone.number.isEmpty) {
                    return 'Please enter your new phone number';
                  }
                  if (!phone.isValidNumber()) return 'Invalid phone number';
                  return null;
                },
              ),
              const SizedBox(height: 20),
              IntlPhoneField(
                controller: _confirmPhoneController,
                initialCountryCode: 'IN',
                keyboardType: TextInputType.phone,
                disableLengthCheck: false,
                decoration: _decoration(
                  label: "Confirm New Phone Number",
                  icon: Icons.phone_outlined,
                ),
                onChanged: (phone) {
                  _confirmCountryCode = phone.countryCode;
                  _confirmPhoneNumber = phone.number;
                },
                validator: (phone) {
                  if (phone == null || phone.number.isEmpty) {
                    return 'Please confirm your new phone number';
                  }
                  if (!phone.isValidNumber()) return 'Invalid phone number';
                  return null;
                },
              ),
              const SizedBox(height: 20),
              TextFormField(
                controller: _currentPasswordController,
                obscureText: _obscureCurrentPassword,
                decoration: _decoration(
                  label: "Current Password",
                  icon: Icons.lock_outline,
                  suffixIcon: IconButton(
                    icon: Icon(
                      _obscureCurrentPassword
                          ? Icons.visibility_off
                          : Icons.visibility,
                      color: AppTheme.primaryGreen,
                    ),
                    onPressed: () {
                      setState(
                        () =>
                            _obscureCurrentPassword = !_obscureCurrentPassword,
                      );
                    },
                  ),
                ),
                validator: (value) {
                  if (value == null || value.isEmpty) {
                    return 'Please enter your current password';
                  }
                  if (value.length < 6) {
                    return 'Password must be at least 6 characters';
                  }
                  return null;
                },
              ),
              const SizedBox(height: 40),
              SizedBox(
                width: double.infinity,
                child: ElevatedButton(
                  onPressed: _isLoading ? null : _updatePhone,
                  style: ElevatedButton.styleFrom(
                    padding: const EdgeInsets.symmetric(vertical: 15),
                    backgroundColor: AppTheme.primaryGreen,
                    shape: RoundedRectangleBorder(
                      borderRadius: BorderRadius.circular(12),
                    ),
                  ),
                  child: _isLoading
                      ? const SizedBox(
                          height: 20,
                          width: 20,
                          child: CircularProgressIndicator(
                            color: AppTheme.bgGradientEnd,
                          ),
                        )
                      : const Text(
                          "Change Phone Number",
                          style: TextStyle(
                            fontSize: 16,
                            color: AppTheme.bgGradientEnd,
                          ),
                        ),
                ),
              ),
            ],
          ),
        ),
      ),
    );
  }
}
